% !TEX root = ../thesis.tex
%
\chapter{Applying optimal control theory to complex models\label{sec:ch4}}

\todo{\begin{enumerate}
    \item More +ve discussion and highlight which problems are real
    \item Lit review of OCT + complex models here?
    \item Discussion of MPC update period
\end{enumerate}}
...
\begin{linenomath*}
    ...
    \end{equation}
\end{linenomath*}
where $S$ and $I$ are numbers of susceptible and infected hosts respectively. Throughout superscripts refer to the risk group (high: H, low: L), and subscripts identify the node. For example, $S_{3}^\mathrm{H}$ represents the total number of high risk susceptibles in node 3. In addition, we use $N$ to refer to the total number of active hosts (\todo{why is this distinction needed?}), such that $N_{3}^\mathrm{H}$ is the number of susceptible, infected and vaccinated high risk hosts in node 3. The sum in Equation~\ref{eqn:ch4:sim_model} is over all connected nodes including the focal node itself, with the relative transmission strength (\todo{rate?}) into node $i$ from node $j$ given by $\sigma_{ij}$, and risk structure given by the \num{2x2} matrix $\rho$.

Mass vaccination is the only intervention we consider, with the potential to target based on both risk group and region but randomised across host infection status (i.e.\ the vaccine is given to all hosts but is only effective on susceptibles). Vaccinated hosts are not susceptible to infection (\todo{i.e.\ the vaccing is totally effective}), and the vaccine does not wane over the time scales considered here. Logistical and economic constraints are included through a maximum total vaccination rate ($\eta_{\mathrm{max}}$) that can be divided between risk groups and regions. Within each group susceptibles are vaccinated at rate $f\eta_{\mathrm{max}}S/N$, where $f$ is the proportion of control allocated to that group, and $N$ (\todo{is this why active distinction needed?}) is the total group population.
...
\begin{figure}[htb]
    \begin{center}
        \includegraphics[width=0.85\textwidth]{Graphics/Ch4/No_Control_DPC}
        \caption{Ten typical simulation model trajectories with no control. Green and red lines represent susceptible and infected hosts respectively. \todo{Add local to top 3 graphs.}}
        \label{fig:ch4:no_control_dpc}
    \end{center}
\end{figure}

Optimal allocation of vaccination resources minimises an epidemic cost $J$ representing the disease burden of the epidemic across all infected hosts over the simulation time ($T$): $J = \int_{t=0}^TI(t)\mathrm{d}t$ (\todo{Has $I$ been defined?}). In common with the particular control we consider and the risk and spatial structures, this simple choice of objective function was made merely to illustrate our methods, but \todo{as shown in Chapters 5 and 7??} the framework generalises immediately to more complex settings.
...
\begin{table}[htb]
    \caption{Parameter values used for simulation model trajectories.\label{tab:ch4:parameters}}
    \begin{tabular}{@{}lll@{}}
        \toprule
...
\begin{linenomath*}
    ...
    \end{equation}
\end{linenomath*}
The \num{3x3} matrix $\tilde{\sigma}$ approximates spatial coupling between regions, and $\tilde{\rho}$ is again a \num{2x2} matrix giving the approximated risk structure (\todo{note the parameters will be different from the risk based model}).

\subsection{Model fitting\label{sec:ch4:model_fitting}}

\todo{Compare quality of fit under control and no control with MLE and least squares. Potentially least squares better fit under no control, but worse with control.}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.75\textwidth]{Graphics/Ch4/Space_Fit}
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption{Profile likelihood analysis for: \textbf{(a)} the risk based approximate model, and \textbf{(b)} the space based approximate model. The offsets ($1.137\times10^6$ for \textbf{(a)} and $9.583\times10^5$ for \textbf{(b)}) must be added to all values on the y axes. The profile for every parameter crosses the 95\% confidence interval threshold (\todo{needs explanation}), ensuring identifiability of each parameter. In all cases maximum likelihood estimates from the unconstrained optimisation coincide with the maximum along the profile.}
...
\todo{Long list of things for future research, but no indication of which is most important, whether can get away without fixing any.}